...
def draw_canvas(canvas, birds, base, pipes, score, gen):
    #draw in canvas
...
def main(genomes, config):
    ...
    while run:
        ...
        #setting pipe focus for the algorithm to check on
        pipe_ind = 0
        if len(birds) > 0:
        ...
        for pipe in pipes:
            for x, bird in enumerate(birds):
                if pipe.collide(bird):
                    #remove birds that collide to pipes
                    ge[x].fitness -= 1
                    birds.pop(x)
                    nets.pop(x)
                    ge.pop(x)
        ...
        base.move()
        draw_canvas(canvas, birds, base, pipes, score, GEN)
...
def run(config_path):
    ...
    #show population stats as the algorithm runs
